...
DECLARE
    ...
    v_total_ordered NUMBER := 0;
BEGIN
    -- Get supplier from receive_master
    SELECT supplier_id INTO v_supplier_id
    FROM product_receive_master
    WHERE receive_id = :NEW.receive_id;

    -- Check if this product was EVER ordered from this supplier
    SELECT NVL(SUM(pod.quantity), 0) INTO v_total_ordered
    FROM product_order_detail pod
    JOIN product_order_master pom ON pod.order_id = pom.order_id
    WHERE pom.supplier_id = v_supplier_id
    AND pod.product_id = :NEW.product_id;
